...
def make_api_request(model, query, encoded_image):
    if model == "meta-llama/llama-prompt-guard-2-22m":
        # This model only supports text input
        simple_messages = [{"role": "user", "content": query}]
        payload = {
            "model": model,
...
def process_image(image_path, query):
    try:
        ...
        llama_11b_response = make_api_request("meta-llama/llama-4-scout-17b-16e-instruct", query, encoded_image)
        llama_90b_response = make_api_request("meta-llama/llama-prompt-guard-2-22m", query, encoded_image)
